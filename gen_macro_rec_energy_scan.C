...
void gen_macro_rec(int gui =0, int runAcc = 0){ 
	...
	int const stepEnergy = 6;
	float momentum[stepEnergy] = {194, 190, 185, 179, 174, 168}; //Unit: MeV
	...
	for (int iEnergy = 0; iEnergy < stepEnergy; ++iEnergy)
	{
		ofs << "# Initialize kernel" << endl;
		ofs << "/run/initialize" << endl;
		ofs << "# Particle Energy" << endl;
		ofs << Form("/tutorial/generator/momentum %f MeV", momentum[iEnergy]) << endl;
		for (int i = 0; i < 1; ++i)
		{
			for (int iX = 0; iX < maxBin; ++iX)
			{
				for (int iY = 0; iY < maxBin; ++iY){
					...
					ofs << "#" << endl;
					ofs << Form("/analysis/setFileName run%d_%d_%d_e_%d", i+1, iX, iY, 20-iEnergy) << endl;
